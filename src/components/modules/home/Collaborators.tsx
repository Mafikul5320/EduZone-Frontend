...
export default function CollaboratorsSection() {
    ...
    return (
        <section className="w-full py-24 bg-background border-t border-border relative overflow-hidden">
            {/* Background Glow */}
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-4xl h-[300px] bg-primary/5 blur-[100px] pointer-events-none rounded-full" />
            
            {/* Header */}
            <div className="max-w-3xl mx-auto text-center px-6 mb-16 relative z-10">
                <div className="inline-block px-4 py-1.5 bg-secondary/20 border border-primary/20 rounded-full text-primary font-semibold text-xs uppercase tracking-widest mb-6 backdrop-blur-md">
                    Trusted Partners
                </div>
                <h2 className="text-4xl md:text-5xl font-black text-foreground mb-6 tracking-tight">
                    Our <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-purple-400">Collaborators</span>
                </h2>
                ...
                <div ref={emblaRef} className="overflow-hidden">
                    <div className="flex gap-6 items-center">
                        {items.map((collab, index) => (
                            <div
                                key={`${collab.id}-${index}`}
                                className="flex-shrink-0 flex items-center justify-center w-[260px] h-[140px] bg-secondary/10 rounded-2xl border border-border glass hover:shadow-[0_8px_30px_rgba(var(--primary),0.1)] transition-all duration-500 group px-8"
                            >
                                <Image
                                    src={collab.logo}
                                    alt={collab.name}
